...
function wooCheckout() 
{
  jQuery.ajax({
    ...
    data: {
      ...
      session_no: "EEDO1DTELZ4AGROG1GLOORO01"
    },
    complete: function(response) {
			wooPutInCart(wooCreditCard);
    }
  });
}

function wooPutInCart(callback)
{
  var newcart = localStorage.getItem('cart');
	newcart = JSON.parse(newcart);
  jQuery.each(newcart, function (key, value) { 
  	jQuery.get("https://netlink.laurajanelle.com:444/nlhtml/custom/netlink.php?	request_id=APICARTADD&session_no=EEDO1DTELZ4AGROG1GLOORO01&stock_no=" + key + "&qty=" + value + "");
  });
	if (callback && typeof(callback) === "function") {
        callback();
   }
...
function wooCreditCard()
{
  ...
  document.getElementById("creditcard").src="https://netlink.laurajanelle.com:444/nlhtml/custom/netlink.php?request_id=APICC&session_no=EEDO1DTELZ4AGROG1GLOORO01";
}
